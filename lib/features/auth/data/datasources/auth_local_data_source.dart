...
class UserPreferences {
  ...
  Future<bool> savePresence(List<Presence> listPresence) async {
    final SharedPreferences prefs = await SharedPreferences.getInstance();
    int countPresent = listPresence
        .where((presence) => presence.status == 'Hadir')
        .length;
    int countSick = listPresence
        .where((presence) => presence.status == 'Sakit')
        .length;
    int countPermission = listPresence
        .where((presence) => presence.status == 'Izin')
        .length;
    int countAbsent = listPresence
        .where((presence) => presence.status == 'Alpa')
        .length;
